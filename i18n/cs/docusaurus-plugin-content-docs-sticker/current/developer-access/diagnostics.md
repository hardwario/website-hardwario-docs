...
Stavová LED signalizuje heartbeat řazený podle závažnosti (stav připojení do sítě, režim rádia, stav alarmu) a jednorázové vzory pro interakci (přístup přes NFC, spuštění vstupu). Definice vzorů viz [**Popis hardwaru**](hardware-description.md#led-indication).
