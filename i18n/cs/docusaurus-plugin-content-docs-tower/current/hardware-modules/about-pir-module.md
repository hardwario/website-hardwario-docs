---
slug: about-pir-module
title: O modulu PIR Module
---
import Image from '@theme/IdealImage';

<div class="container">
  <div class="row">
    <div class="col col--4">
      <div><Image img={require('../../../../../tower/hardware-modules/images/pir-module.png')} alt="Čelní panel modulu PIR Module s bílou kopulovitou čočkou pasivního infračerveného detektoru pohybu" /></div>
    </div>
    <div class="col col--6">
      <p>
        Modul <b>PIR Module</b> funguje jako <b>detektor pohybu</b> pracující na <b>pasivním infračerveném principu (PIR)</b>. Typickým použitím modulu PIR Module může být bezdrátový detektor pohybu umístěný na stěně nebo na stropě.
      </p>
      <p>
        Modul je osazen <b>digitálním senzorem s extrémně nízkou spotřebou</b> z rodiny <b>Excelitas’ DigiPyro®</b>.
      </p>
    </div>
  </div>
</div>

## Vlastnosti {#features}
- Digitální PIR detektor **PYQ-1648-7053 (Excelitas)**
- Plně **digitální zpracování** (analýza signálu probíhá v senzoru)
- Čtyřprvkový typ senzoru
- Všesměrová čočka Carclo Diamond 37
- Rozlišení vnitřního A/D převodníku: 14 bitů
- Detekční úhel: 98° (horizontálně i vertikálně)
- Maximální detekční vzdálenost: 5 m
- Spotřeba: přibližně 3μA
- Rozsah napájecího napětí: 2,5V až 3,6V
- Rozsah pracovních teplot: -20 až 70 °C
- Rozměry: 33 x 55 mm

## Odkazy {#references}
- [**Obchod**](https://www.hardwario.store/p/pir-module)
- [**Schémata**](https://github.com/hardwario/bc-hardware/tree/master/out/bc-module-pir)
- [**Knihovna SDK**](https://sdk.hardwario.com/group__twr__module__pir)
- [**Hlavičkový soubor**](https://github.com/hardwario/twr-sdk/blob/master/twr/inc/twr_module_pir.h)
- [**Zdrojový soubor**](https://github.com/hardwario/twr-sdk/blob/master/twr/src/twr_module_pir.c)
- [**Projekty**](https://www.hackster.io/hardwario/projects?part_id=73716)
