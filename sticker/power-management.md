...
Device parameters, keys, and `radio-mode` activation are managed wirelessly using an NFC-enabled smartphone running [**HARDWARIO Manager**](/sticker/hardwario-manager/), even before batteries are installed.
:::
...
- **Field Activation:** Radio transmission (`radio-mode on`) is enabled on-site during commissioning by tapping the unit with [**HARDWARIO Manager**](/sticker/hardwario-manager/) over NFC or via shell commands (`config radio-mode on`).
...
- **RF Energy Harvesting:** The RF field generated by a smartphone running [**HARDWARIO Manager**](/sticker/hardwario-manager/) supplies sufficient power to write parameters directly to the onboard NFC tag EEPROM.
